...
if [ "$1" != "gradejustinstall" ]; then
    ...
    if ! ping -c 1 google.com >/dev/null 2>&1; then
        ...
        # clear
        printf "$GREEN$BOLD"
        echo -ne "✔$RESET version$GREEN$BOLD v$version\n\n"
...
while [ ! -f .system/a.out ]; do
    for i in "${spin[@]}"; do
        echo -ne "$LINE_UP$WHITE$i$WHITE$BOLD Compilation of$BOLD$MANGENTA 1337_Exam_Simulation	$RESET\n"
        if [ -f .system/.devmake.err ]; then
...
printf "$LINE_UP$CLEAR_LINE$GREEN$BOLD"
echo -ne "✔$RESET Compilation of$BOLD$MANGENTA 1337_Exam_Simulation $RESET\n"
...
if [ -z "$USER" ]; then
    ...
    export USER="SegFault777" # Créer la variable d'environnement USER
